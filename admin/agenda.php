...
$act=$_POST['act']??'save';
if($act==='bulk_delete'){ $ids=array_filter(array_map('intval',(array)($_POST['ids']??[]))); if(!$ids){ Session::flash('err','Pilih minimal 1 data.'); } else { $ph=implode(',',array_fill(0,count($ids),'?')); $st=$db->prepare("SELECT title FROM agenda WHERE id IN ($ph)"); $st->execute(array_values($ids)); $names=$st->fetchAll(PDO::FETCH_COLUMN); $db->prepare("DELETE FROM agenda WHERE id IN ($ph)")->execute(array_values($ids)); Auth::log($db,'delete','agenda','Hapus agenda: '.implode(', ',array_slice($names,0,3)).(count($names)>3?' (+'.(count($names)-3).' lain)':'')); Session::flash('ok',count($ids).' data dihapus.'); } }
elseif($act==='delete'){ $eid=(int)$_POST['id']; $en=$db->prepare("SELECT title FROM agenda WHERE id=?"); $en->execute([$eid]); $ename=$en->fetchColumn()?:'agenda#'.$eid; $db->prepare("DELETE FROM agenda WHERE id=?")->execute([$eid]); Auth::log($db,'delete','agenda',"Hapus agenda $ename"); Session::flash('ok','Dihapus.'); }
...
if(!empty($_POST['id'])){ $db->prepare("UPDATE agenda SET title=?,event_date=?,end_date=?,start_time=?,end_time=?,location=?,description=?,status=? WHERE id=?")->execute([$t,$sd,$ed,$_POST['start_time']??'',$_POST['end_time']??'',$_POST['location']??'',$_POST['description']??'',$_POST['status']??'published',(int)$_POST['id']]); Auth::log($db,'update','agenda',"Ubah agenda $t ($sd)".($loc?" di $loc":'')); } else { $db->prepare("INSERT INTO agenda(title,event_date,end_date,start_time,end_time,location,description,status) VALUES(?,?,?,?,?,?,?,?)")->execute([$t,$sd,$ed,$_POST['start_time']??'',$_POST['end_time']??'',$_POST['location']??'',$_POST['description']??'',$_POST['status']??'published']); Auth::log($db,'create','agenda',"Tambah agenda $t ($sd)".($loc?" di $loc":'')); }
