...
$act=$_POST['act']??'save';
if($act==='bulk_delete'){ $ids=array_filter(array_map('intval',(array)($_POST['ids']??[]))); if(!$ids){ Session::flash('err','Pilih minimal 1 data.'); } else { $ph=implode(',',array_fill(0,count($ids),'?')); $st=$db->prepare("SELECT title FROM announcements WHERE id IN ($ph)"); $st->execute(array_values($ids)); $names=$st->fetchAll(PDO::FETCH_COLUMN); $db->prepare("DELETE FROM announcements WHERE id IN ($ph)")->execute(array_values($ids)); Auth::log($db,'delete','announcements','Hapus pengumuman: '.implode(', ',array_slice($names,0,3)).(count($names)>3?' (+'.(count($names)-3).' lain)':'')); Session::flash('ok',count($ids).' data dihapus.'); } }
elseif($act==='delete'){ $eid=(int)$_POST['id']; $en=$db->prepare("SELECT title FROM announcements WHERE id=?"); $en->execute([$eid]); $ename=$en->fetchColumn()?:'pengumuman#'.$eid; $db->prepare("DELETE FROM announcements WHERE id=?")->execute([$eid]); Auth::log($db,'delete','announcements',"Hapus pengumuman $ename"); Session::flash('ok','Dihapus.'); }
...
$stt=$_POST['status']??'published';
if(!empty($_POST['id'])){ $db->prepare("UPDATE announcements SET title=?,content=?,attachment=?,status=?,published_at=? WHERE id=?")->execute([$t,$_POST['content']??'',$att,$stt,$_POST['published_at']?:date('Y-m-d H:i:s'),(int)$_POST['id']]); Auth::log($db,'update','announcements',"Ubah pengumuman $t ($stt)".($att?' + lampiran':'')); } else { $db->prepare("INSERT INTO announcements(title,content,attachment,status,published_at,author_id) VALUES(?,?,?,?,?,?)")->execute([$t,$_POST['content']??'',$att,$stt,$_POST['published_at']?:date('Y-m-d H:i:s'),$_SESSION['user']['id']]); Auth::log($db,'create','announcements',"Tambah pengumuman $t ($stt)".($att?' + lampiran':'')); }
