...
<!-- Baris 1: Grafik Berita berdampingan dengan Pengumuman -->
<div class="grid lg:grid-cols-2 gap-3 mt-3">
  <div class="bg-white rounded-2xl border p-4 flex flex-col">
    <div class="flex items-center justify-between mb-2">
      <h2 class="font-bold text-sm"><i class="fa fa-chart-column text-emerald-600 mr-1"></i>Grafik Berita (6 bulan)</h2>
      <span class="text-[11px] text-slate-500">Published vs Draft</span>
    </div>
    <div class="h-64 sm:h-72 flex-1"><canvas id="newsChart"></canvas></div>
  </div>

  <div class="relative overflow-hidden rounded-2xl border p-4 bg-gradient-to-br from-rose-600 via-pink-600 to-fuchsia-600 text-white flex flex-col">
    <span class="absolute -right-10 -top-12 w-44 h-44 rounded-full border-[18px] border-white/10 pointer-events-none"></span>
    <span class="absolute -left-14 -bottom-16 w-52 h-52 rounded-full border-[22px] border-white/10 pointer-events-none"></span>
    <div class="relative flex flex-wrap items-center gap-2 mb-2">
      <h2 class="font-bold text-sm"><i class="fa fa-bullhorn mr-1"></i>Pengumuman Terbaru</h2>
      <span class="text-[11px] bg-white text-rose-700 px-2 py-0.5 rounded-full font-bold"><?= $nAnn ?> info</span>
      <?php if($nAnnFile): ?><span class="text-[11px] bg-white/20 border border-white/30 px-2 py-0.5 rounded-full font-bold"><i class="fa fa-paperclip mr-1"></i><?= $nAnnFile ?> lampiran</span><?php endif; ?>
      <a href="<?= Helper::url('admin/announcements') ?>" class="ml-auto text-[11px] bg-white/20 border border-white/30 px-2.5 py-1 rounded-lg font-bold hover:bg-white/30">Kelola →</a>
    </div>
    <div class="relative flex-1 overflow-y-auto max-h-[300px] pr-1">
      <?php if(!$anns): ?>
        <div class="py-10 text-center"><span class="w-11 h-11 rounded-2xl bg-white/20 grid place-items-center mx-auto"><i class="fa fa-bell-slash"></i></span><p class="text-sm text-white/90 mt-2 font-semibold">Belum ada pengumuman</p></div>
      <?php else: foreach($anns as $i=>$an): $dt=$an['published_at']??$an['created_at']; $hasFile=!empty($an['attachment']); ?>
        <a href="<?= Helper::url('admin/announcements') ?>" class="flex gap-2.5 items-start bg-white text-slate-800 rounded-xl px-3 py-2.5 mb-2 hover:-translate-y-0.5 hover:shadow-lg transition">
          ...
          <span class="min-w-0 flex-1">
            ...
            <span class="text-xs text-slate-500 block truncate mt-0.5"><?= Helper::e(Helper::excerpt($an['content']??'',90)) ?></span>
          </span>
          <span class="self-center text-rose-500 font-bold text-sm shrink-0">→</span>
        </a>
...
<!-- Baris 2: Agenda berdampingan dengan Aktivitas -->
<div class="grid lg:grid-cols-2 gap-3 mt-3">
  <div class="bg-white rounded-2xl border p-4 flex flex-col">
    <div class="flex flex-wrap items-center gap-2 mb-2">
      <h2 class="font-bold text-sm"><i class="fa fa-calendar-days text-violet-500 mr-1"></i>Agenda Terdekat</h2>
      <span class="text-[11px] bg-violet-100 text-violet-700 px-2 py-0.5 rounded-full font-bold"><?= $nAgenda ?> agenda</span>
      <a href="<?= Helper::url('admin/agenda') ?>" class="ml-auto text-[11px] text-emerald-600 font-bold">Kelola →</a>
    </div>
    <div class="flex-1 overflow-y-auto h-[380px] pr-1" style="scrollbar-width: thin; scrollbar-color: #cbd5e1 transparent;">
      <?php if(!$upcoming): ?>
        <div class="py-10 text-center"><span class="w-11 h-11 rounded-2xl bg-slate-100 text-slate-400 grid place-items-center mx-auto"><i class="fa fa-calendar-xmark"></i></span><p class="text-sm text-slate-500 mt-2 font-semibold">Belum ada agenda mendatang</p></div>
      <?php else: foreach($upcoming as $a): $edEnd=(($a['end_date']??'')!=='')?$a['end_date']:$a['event_date']; $dd=(int)floor((strtotime($a['event_date'])-strtotime(date('Y-m-d')))/86400); $ddEnd=(int)floor((strtotime($edEnd)-strtotime(date('Y-m-d')))/86400); $when=$ddEnd<0?'Selesai':(($dd<=0&&$ddEnd>=0)?'Berlangsung':($dd===1?'Besok':($dd<=0?'Hari ini':"H-$dd"))); ?>
        <div class="flex gap-2.5 items-start border border-slate-100 bg-slate-50 rounded-xl px-3 py-2.5 mb-2">
          ...
          <span class="min-w-0 flex-1">
            <b class="text-sm block truncate"><?= Helper::e($a['title']) ?></b>
            ...
            <?php if(!empty($a['description'])): ?><span class="text-xs text-slate-500 block truncate mt-0.5"><?= Helper::e($a['description']) ?></span><?php endif; ?>
          </span>
          <span class="text-[10px] font-bold px-2 py-0.5 rounded-full shrink-0 <?= $dd===0?'bg-red-100 text-red-700':'bg-violet-100 text-violet-700' ?>"><?= $when ?></span>
        </div>
      <?php endforeach; endif; ?>
    </div>
  </div>
  ...
  <div class="bg-white rounded-2xl border p-4 flex flex-col">
    <div class="flex flex-wrap items-center gap-2 mb-3">
      <h2 class="font-bold text-sm"><i class="fa fa-clock-rotate-left text-sky-600 mr-1"></i>Aktivitas Terbaru</h2>
      <span class="text-[11px] bg-slate-800 text-white px-2.5 py-0.5 rounded-full font-bold"><?= $total24 ?> / 24 jam</span>
      <?php if($recent): ?>
        <span class="text-[11px] bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full font-bold">+<?= $nAdd ?></span>
        <span class="text-[11px] bg-sky-100 text-sky-700 px-2 py-0.5 rounded-full font-bold">✎<?= $nEdit ?></span>
        <span class="text-[11px] bg-red-100 text-red-700 px-2 py-0.5 rounded-full font-bold">🗑<?= $nDel ?></span>
        <span class="text-[11px] bg-violet-100 text-violet-700 px-2 py-0.5 rounded-full font-bold"><?= $uniqUser ?> user</span>
      <?php endif; ?>
      <span class="ml-auto text-[11px] text-slate-400"><i class="fa fa-trash-can mr-1"></i>24 jam</span>
    </div>
    <div class="flex-1 min-h-0">
      <?php if(!$recent): ?>
        <div class="py-10 text-center"><span class="w-12 h-12 rounded-2xl bg-slate-100 grid place-items-center mx-auto text-slate-400"><i class="fa fa-mug-saucer text-xl"></i></span><p class="text-sm text-slate-500 mt-2 font-semibold">Belum ada aktivitas 24 jam terakhir</p><p class="text-xs text-slate-400">Setiap aksi admin tercatat di sini.</p></div>
      <?php else: ?>
        <div class="relative pl-10 h-[380px] overflow-y-auto pr-2 py-1 scroll-smooth" style="scrollbar-width: thin; scrollbar-color: #cbd5e1 transparent;">
