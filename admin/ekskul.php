...
if($act==='bulk_delete'){ $ids=array_filter(array_map('intval',(array)($_POST['ids']??[]))); if(!$ids){ Session::flash('err','Pilih minimal 1 data.'); } else { $ph=implode(',',array_fill(0,count($ids),'?')); $st=$db->prepare("SELECT name FROM extracurriculars WHERE id IN ($ph)"); $st->execute(array_values($ids)); $names=$st->fetchAll(PDO::FETCH_COLUMN); $db->prepare("DELETE FROM extracurriculars WHERE id IN ($ph)")->execute(array_values($ids)); Auth::log($db,'delete','ekskul','Hapus ekskul: '.implode(', ',array_slice($names,0,3)).(count($names)>3?' (+'.(count($names)-3).' lain)':'')); Session::flash('ok',count($ids).' data dihapus.'); } }
elseif($act==='delete'){ $eid=(int)$_POST['id']; $en=$db->prepare("SELECT name FROM extracurriculars WHERE id=?"); $en->execute([$eid]); $ename=$en->fetchColumn()?:'ekskul#'.$eid; $db->prepare("DELETE FROM extracurriculars WHERE id=?")->execute([$eid]); Auth::log($db,'delete','ekskul',"Hapus ekskul $ename"); Session::flash('ok','Dihapus.'); }
...
if(!empty($_POST['id'])){ $db->prepare("UPDATE extracurriculars SET name=?,description=?,coach=?,`day`=?,`time`=?,schedule=? WHERE id=?")->execute([$nm,$_POST['description']??'',$coach,$day,$time,$_POST['schedule']??'',(int)$_POST['id']]); Auth::log($db,'update','ekskul',"Ubah ekskul $nm".($coach?" (pembina: $coach)":'').($day?" - $day":'')); } else { $db->prepare("INSERT INTO extracurriculars(name,description,coach,`day`,`time`,schedule) VALUES(?,?,?,?,?,?)")->execute([$nm,$_POST['description']??'',$coach,$day,$time,$_POST['schedule']??'']); Auth::log($db,'create','ekskul',"Tambah ekskul $nm".($coach?" (pembina: $coach)":'').($day?" - $day":'')); }
