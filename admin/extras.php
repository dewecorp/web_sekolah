...
elseif($act==='delete'){ $eid=(int)$_POST['id']; $col=$t==='prestasi'?'title':'name'; $en=$db->prepare("SELECT $col FROM $tbl WHERE id=?"); $en->execute([$eid]); $ename=$en->fetchColumn()?:$t.'#'.$eid; $db->prepare("DELETE FROM $tbl WHERE id=?")->execute([$eid]); Auth::log($db,'delete',$t,"Hapus $t $ename"); Session::flash('ok','Dihapus.'); }
