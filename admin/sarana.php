...
$act=$_POST['act']??'save';
if($act==='meta'){ foreach(['sarana_title','sarana_desc','sarana_show'] as $k){ if(!array_key_exists($k,$_POST['s']??[])) continue; $v=trim((string)$_POST['s'][$k]); if($k==='sarana_show') $v=$v==='1'?'1':'0'; $db->prepare("INSERT INTO settings(`key`,`value`) VALUES(?,?) ON DUPLICATE KEY UPDATE `value`=VALUES(`value`)")->execute([$k,$v]); } Auth::log($db,'update','sarana','Ubah pengaturan'); Session::flash('ok','Pengaturan disimpan.'); }
elseif($act==='bulk_delete'){ $ids=array_filter(array_map('intval',(array)($_POST['ids']??[]))); if(!$ids){ Session::flash('err','Pilih minimal 1 data.'); } else { $ph=implode(',',array_fill(0,count($ids),'?')); $st=$db->prepare("SELECT name FROM facilities WHERE id IN ($ph)"); $st->execute(array_values($ids)); $names=$st->fetchAll(PDO::FETCH_COLUMN); $db->prepare("DELETE FROM facilities WHERE id IN ($ph)")->execute(array_values($ids)); Auth::log($db,'delete','sarana','Hapus sarana: '.implode(', ',array_slice($names,0,3)).(count($names)>3?' (+'.(count($names)-3).' lain)':'')); Session::flash('ok',count($ids).' data dihapus.'); } }
elseif($act==='delete'){ $eid=(int)$_POST['id']; $en=$db->prepare("SELECT name FROM facilities WHERE id=?"); $en->execute([$eid]); $ename=$en->fetchColumn()?:'sarana#'.$eid; $db->prepare("DELETE FROM facilities WHERE id=?")->execute([$eid]); Auth::log($db,'delete','sarana',"Hapus sarana $ename"); Session::flash('ok','Dihapus.'); }
...
if(!empty($_POST['id'])){ $db->prepare("UPDATE facilities SET name=?,qty=?,unit=?,cond=?,sort_order=? WHERE id=?")->execute([$nm,$qty,$unit,$cond,$so,(int)$_POST['id']]); Auth::log($db,'update','sarana',"Ubah sarana $nm ($qty $unit, $cond)"); } else { $db->prepare("INSERT INTO facilities(name,qty,unit,cond,sort_order) VALUES(?,?,?,?,?)")->execute([$nm,$qty,$unit,$cond,$so]); Auth::log($db,'create','sarana',"Tambah sarana $nm ($qty $unit, $cond)"); }
