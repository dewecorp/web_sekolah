...
$act=$_POST['act']??'save';
if($act==='meta'){ foreach(['siswa_title','siswa_desc','siswa_show','siswa_cols'] as $k){ if(!array_key_exists($k,$_POST['s']??[])) continue; $v=trim((string)$_POST['s'][$k]); if($k==='siswa_show') $v=$v==='1'?'1':'0'; $db->prepare("INSERT INTO settings(`key`,`value`) VALUES(?,?) ON DUPLICATE KEY UPDATE `value`=VALUES(`value`)")->execute([$k,$v]); } Session::flash('ok','Pengaturan siswa disimpan.'); }
elseif($act==='class_save'){ $cn=trim($_POST['class_name']??''); $nl=max(0,(int)($_POST['n_l']??0)); $np=max(0,(int)($_POST['n_p']??0)); if($cn===''){ Session::flash('err','Nama kelas wajib.'); } else { $so=(int)($_POST['sort_order']??0); if(!empty($_POST['id'])){ $cid=(int)$_POST['id']; $db->prepare("UPDATE student_classes SET name=?,n_l=?,n_p=?,sort_order=? WHERE id=?")->execute([$cn,$nl,$np,$so,$cid]); } else { try{ $db->prepare("INSERT INTO student_classes(name,n_l,n_p,sort_order) VALUES(?,?,?,?)")->execute([$cn,$nl,$np,$so]); }catch(Throwable){ Session::flash('err','Kelas sudah ada.'); header('Location: '.Helper::url('admin/students')); exit; } } Session::flash('ok','Kelas disimpan.'); } }
elseif($act==='class_del'){ $db->prepare("DELETE FROM student_classes WHERE id=?")->execute([(int)$_POST['id']]); Session::flash('ok','Kelas dihapus (siswa jadi tanpa kelas).'); }
elseif($act==='bulk_delete'){ $ids=array_filter(array_map('intval',(array)($_POST['ids']??[]))); if(!$ids){ Session::flash('err','Pilih minimal 1 data.'); } else { $ph=implode(',',array_fill(0,count($ids),'?')); $db->prepare("DELETE FROM student_classes WHERE id IN ($ph)")->execute(array_values($ids)); Session::flash('ok',count($ids).' kelas dihapus.'); } }
elseif($act==='delete'){ $db->prepare("DELETE FROM student_classes WHERE id=?")->execute([(int)$_POST['id']]); Session::flash('ok','Dihapus.'); }
