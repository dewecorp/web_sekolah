...
$act=$_POST['act']??'save';
if($act==='delete'){ $eid=(int)$_POST['id']; $en=$db->prepare("SELECT title,type,area FROM widgets WHERE id=?"); $en->execute([$eid]); $er=$en->fetch(); $ename=trim(($er['title']??'').' ('.($er['type']??'').'/'.($er['area']??'').')')?:'widget#'.$eid; $db->prepare("DELETE FROM widgets WHERE id=?")->execute([$eid]); Auth::log($db,'delete','widgets',"Hapus widget $ename"); Session::flash('ok','Dihapus.'); }
elseif($act==='move'){
  ...
  if($type==='image'){ $il=trim((string)($_POST['image_link']??'')); $ib=!empty($_POST['image_blank'])?1:0; if($il!=='') $content=json_encode(['src'=>$content,'link'=>$il,'blank'=>$ib]); }
  if(!empty($_POST['id'])){ $db->prepare("UPDATE widgets SET area=?,type=?,title=?,content=?,animation=?,sort_order=?,is_active=? WHERE id=?")->execute([$area,$type,$tt,$content,$animation,(int)($_POST['sort_order']??0),(int)($_POST['is_active']??1),(int)$_POST['id']]); Auth::log($db,'update','widgets',"Ubah widget $tt ($type/$area)"); } else { $db->prepare("INSERT INTO widgets(area,type,title,content,animation,sort_order,is_active) VALUES(?,?,?,?,?,?,?)")->execute([$area,$type,$tt,$content,$animation,(int)($_POST['sort_order']??0),(int)($_POST['is_active']??1)]); Auth::log($db,'create','widgets',"Tambah widget $tt ($type/$area)"); } Session::flash('ok','Disimpan.'); }
header('Location: '.Helper::url($redirectAdmin)); exit; }
...
if(!parsed.length){buildLinkRow();return;}parsed.forEach((l,i)=>buildLinkRow(l.label||'',l.url||'',(l.target||'')==='_blank'||document.querySelectorAll('input[name="link_blank[]"]')[i]?.checked));}
function serializeLinks(){const rows=document.querySelectorAll('#linksList > div');const data=[];rows.forEach(r=>{const l=r.querySelector('input[name="link_label[]"]')?.value||'';const u=r.querySelector('input[name="link_url[]"]')?.value||'';const b=r.querySelector('input[name="link_blank[]"]')?.checked;if(l||u)data.push({label:l,url:u,target:b?'_blank':'_self'});});return JSON.stringify(data);}
