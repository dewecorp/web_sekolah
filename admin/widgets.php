...
function toggleImageField(){const type=document.getElementById('f_type').value,image=type==='image',hasContent=editorTypes.includes(type);document.getElementById('imageField').classList.toggle('hidden',!image);document.getElementById('animationField').classList.toggle('hidden',!image);document.getElementById('contentField').classList.toggle('hidden',!hasContent);document.getElementById('contentLabel').textContent=type==='announcements'?'Pengantar Pengumuman':'Konten';document.getElementById('contentHelp').textContent=type==='announcements'?'Teks ini tampil di atas daftar otomatis pada widget.':'Bisa berisi teks terformat, tautan, atau HTML.';if(hasContent&&!modal.classList.contains('hidden'))ensureWidgetEditor()}
