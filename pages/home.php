...
<?php elseif($type==='countdown'): $target=trim((string)($s['content']??'')); ?>
<div class="cta-panel relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-violet-700 via-indigo-600 to-blue-600 text-white p-7 md:p-12 shadow-xl"><span class="absolute -right-16 -top-20 w-64 h-64 rounded-full border-[28px] border-white/10"></span><span class="absolute -left-20 -bottom-24 w-72 h-72 rounded-full border-[36px] border-white/10"></span><div class="relative max-w-3xl mx-auto text-center"><span class="inline-flex items-center gap-2 rounded-full bg-white/15 border border-white/20 px-3 py-1 text-[11px] font-bold uppercase tracking-[.16em]"><i class="fa fa-clock text-amber-300"></i>Countdown</span><h3 class="text-3xl md:text-5xl font-extrabold leading-tight mt-4"><?= Helper::e($s['title']??'Countdown') ?></h3><p class="text-white/80 text-sm md:text-base max-w-2xl mx-auto mt-4"><?= Helper::e($s['subtitle']??'') ?></p><div id="cd-<?= (int)$s['id'] ?>" class="mt-7 flex flex-wrap justify-center gap-4" data-target="<?= Helper::e($target) ?>"></div><div class="mt-7 flex flex-wrap justify-center gap-3"><?= $secBtns($s) ?></div></div></div>
<?php elseif($type==='image'): $simg=Helper::cover($s['image']??'','image-'.($s['id']??0),1600,900); ?>
<div class="relative rounded-[2rem] overflow-hidden card-hover"><img src="<?= Helper::e($simg) ?>" alt="<?= Helper::e($s['title']??'') ?>" class="w-full h-auto object-cover" loading="lazy"><?php if(($s['title']??'')||($s['subtitle']??'')): ?><div class="absolute inset-x-0 bottom-0 bg-gradient-to-t from-slate-950/90 via-slate-900/30 to-transparent p-6 md:p-10 text-white"><h3 class="text-2xl md:text-4xl font-extrabold"><?= Helper::e($s['title']) ?></h3><?php if($s['subtitle']): ?><p class="mt-2 text-white/80 max-w-2xl"><?= Helper::e($s['subtitle']) ?></p><?php endif; ?><div class="mt-4"><?= $secBtns($s) ?></div></div><?php endif; ?></div>
<?php elseif($type==='video'): $vids=array_values(array_filter(array_map(fn($x)=>['title'=>$x['heading']??'','url'=>trim((string)($x['image']??$x['subheading']??''))], $secSlides[$s['id']??0]??[]),fn($x)=>$x['url']!=='')); foreach($vids as &$vv){ if(preg_match('~(?:youtube\.com/(?:watch\?v=|shorts/)|youtu\.be/)([\w-]{6,})~',$vv['url'],$mm))$vv['url']='https://www.youtube.com/embed/'.$mm[1]; } unset($vv); if(!$vids&&trim((string)($s['content']??''))!==''){ $one=trim((string)$s['content']); if(preg_match('~(?:youtube\.com/(?:watch\?v=|shorts/)|youtu\.be/)([\w-]{6,})~',$one,$mm))$one='https://www.youtube.com/embed/'.$mm[1]; $vids=[['title'=>$s['title']??'','url'=>$one]]; } ?>
<?php if(!$vids): ?><p class="opacity-70 text-sm">Belum ada video. Tambah via tab Video.</p><?php else: $first=array_shift($vids); ?>
<div class="relative rounded-[2rem] overflow-hidden bg-slate-950 max-w-4xl mx-auto w-full"><div class="aspect-video w-full max-h-[420px]"><iframe id="vid-main-<?= (int)$s['id'] ?>" src="<?= Helper::e($first['url']) ?>" class="w-full h-full" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen loading="lazy" title="<?= Helper::e($first['title']?:($s['title']??'Video')) ?>"></iframe></div><div class="p-5 md:p-6 text-white flex flex-col sm:flex-row sm:items-center gap-2"><div class="min-w-0"><h3 id="vid-title-<?= (int)$s['id'] ?>" class="text-xl md:text-2xl font-extrabold truncate"><?= Helper::e($first['title']?:($s['title']??'Video')) ?></h3><?php if($s['subtitle']): ?><p class="mt-1 text-sm text-white/70"><?= Helper::e($s['subtitle']) ?></p><?php endif; ?></div><span class="sm:ml-auto text-xs font-bold px-2.5 py-1 rounded-full bg-white/15 border border-white/20 shrink-0"><i class="fa fa-play mr-1"></i><?= count($vids)+1 ?> video</span></div></div>
<?php if($vids): ?><div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 mt-3 max-w-5xl mx-auto"><?php foreach($vids as $vi=>$vv): ?><button type="button" data-vid-play="<?= Helper::e($vv['url']) ?>" data-vid-title="<?= Helper::e($vv['title']?:('Video '.($vi+2))) ?>" data-vid-target="vid-main-<?= (int)$s['id'] ?>" data-vid-label="vid-title-<?= (int)$s['id'] ?>" class="flex items-center gap-3 rounded-2xl border dark:border-slate-700 bg-white dark:bg-slate-800 p-3 text-left card-hover"><span class="w-11 h-11 rounded-xl bg-slate-950 text-white grid place-items-center shrink-0"><i class="fa fa-play text-xs"></i></span><span class="min-w-0"><b class="block text-sm truncate"><?= Helper::e($vv['title']?:('Video '.($vi+2))) ?></b><span class="block text-[11px] text-slate-400 truncate"><?= Helper::e(preg_replace('~^https?://(www\.)?~','',$vv['url'])) ?></span></span></button><?php endforeach; ?></div><?php endif; ?>
<?php endif; ?>
