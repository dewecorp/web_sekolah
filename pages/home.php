...
<style>@keyframes galSld{from{transform:translateX(0)}to{transform:translateX(-50%)}}#galWrap<?= (int)$s['id'] ?>{overflow-x:clip;overflow-y:visible;padding:3rem 2rem;margin:-1.5rem -1rem;mask-image:linear-gradient(to right,transparent,#000 10%,#000 90%,transparent)}#galTrk<?= (int)$s['id'] ?>{display:flex;gap:1.25rem;width:max-content;padding:.6rem 1rem;animation:galSld <?= $galDur ?> linear infinite<?= $galAnim==='marquee-alt'?' reverse':'' ?>}#galTrk<?= (int)$s['id'] ?>:hover{animation-play-state:paused}#galWrap<?= (int)$s['id'] ?> img{transition:transform .4s ease,box-shadow .4s ease,filter .4s ease}#galWrap<?= (int)$s['id'] ?> img.gal-glow{position:relative;transform:scale(1.18);box-shadow:0 18px 34px rgba(16,185,129,.32),0 6px 14px rgba(15,23,42,.18),0 0 0 3px rgba(255,255,255,.9),0 0 34px rgba(16,185,129,.5);filter:saturate(1.15) brightness(1.06);z-index:5}</style>
