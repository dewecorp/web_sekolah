...
<?php endforeach; ?></div><?php endif; ?>
<?php elseif($type==='galeri'): $rows=array_slice($galImgs,0,max(4,$lim)); $galGrid=$s['grid']??'cards-4'; $galCols=$galGrid==='cards-2'?'grid-cols-2':($galGrid==='cards-3'?'grid-cols-2 md:grid-cols-3':'grid-cols-2 md:grid-cols-4'); ?>
<?php if(!$rows): ?><div class="grid <?= $galCols ?> gap-3"><?php for($gi=0;$gi<4;$gi++): ?><img src="<?= Helper::e(Helper::dummy('galeri-'.$gi.'-'.($s['section_key']??''),600,450)) ?>" alt="Galeri sekolah" loading="lazy" class="h-40 w-full object-cover rounded-xl border card-hover"><?php endfor; ?></div><?php else: ?><div class="grid <?= $galCols ?> gap-3"><?php foreach($rows as $g): ?><img src="<?= Helper::url($g['filepath']) ?>" alt="<?= Helper::e($g['caption']??$g['gtitle']) ?>" data-lightbox loading="lazy" class="h-40 w-full object-cover rounded-xl border cursor-zoom-in card-hover"><?php endforeach; ?></div><?php endif; ?>
