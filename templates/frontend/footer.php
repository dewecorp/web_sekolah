...
function renderFw($fw,$db){
  ...
  if($t==='about'){
    ...
    echo '<h4 class="font-bold text-white mb-2">'.Helper::e($school).'</h4><p class="text-white/85">'.Helper::e($tagline).'</p><p class="mt-2 text-white/75">'.Helper::e($addr).'</p>';
    return;
  }
  if($t==='image'&&$fwc!==''){ $fl=''; $fb=false; try{ $pj=json_decode($fwc,true); if(is_array($pj)&&!empty($pj['src'])){ $fwc=$pj['src']; $fl=trim($pj['link']??''); $fb=!empty($pj['blank']); } }catch(Throwable){} $fwi=(preg_match('~^(?:https?:)?//~i',$fwc)||str_starts_with($fwc,'/'))?$fwc:Helper::upload($fwc); $img='<img src="'.Helper::e($fwi).'" alt="'.Helper::e($fw['title']??'').'" class="max-w-full max-h-36 rounded-xl object-contain">'; if($fl!==''){ [$fh,$au]=Helper::menuUrl($fl); $tg=(($fb||$au==='_blank')?' target="_blank" rel="noopener noreferrer"':''); echo '<a href="'.Helper::e($fh).'"'.$tg.'>'.$img.'</a>'; } else echo $img; return; }
  if($t==='menu'){ $menuId=(int)$fwc; $menuItems=[]; try{ if($menuId>0){$mi=$db->prepare("SELECT * FROM menu_items WHERE menu_id=? AND is_active=1 ORDER BY sort_order");$mi->execute([$menuId]);$menuItems=$mi->fetchAll();} }catch(Throwable){} if(!$menuItems){try{$fm=$db->query("SELECT id FROM menus WHERE location='footer' LIMIT 1")->fetch();if($fm){$mi2=$db->prepare("SELECT * FROM menu_items WHERE menu_id=? AND is_active=1 ORDER BY sort_order");$mi2->execute([(int)$fm['id']]);$menuItems=$mi2->fetchAll();}}catch(Throwable){}} echo '<div class="grid gap-1.5">'; foreach($menuItems as $fi){[$fhref,$ftgt]=Helper::menuUrl($fi['url']); echo '<a href="'.Helper::e($fhref).'"'.($ftgt==='_blank'?' target="_blank" rel="noopener noreferrer"':'').' class="text-sm text-white/85 hover:text-white hover:translate-x-1 transition"><i class="fa fa-chevron-right mr-2 text-white"></i>'.Helper::e($fi['label']).'</a>'; } echo '</div>'; return; }
  if($t==='categories'){ try{$catsF=$db->query("SELECT name,slug FROM categories ORDER BY name")->fetchAll();}catch(Throwable){$catsF=[];} echo '<div class="grid gap-1.5">'; foreach($catsF as $cf) echo '<a href="'.Helper::url('berita?kategori='.Helper::e($cf['slug'])).'" class="text-sm text-white/85 hover:text-white hover:translate-x-1 transition"><i class="fa fa-tag mr-2 text-white"></i>'.Helper::e($cf['name']).'</a>'; echo '</div>'; return; }
  if($t==='links'){ $lnks=json_decode((string)$fwc,true)?:[]; if(!$lnks&&$fwc!==''){ $lnks=array_map(fn($l)=>['label'=>trim(explode('|',$l)[0]??''),'url'=>trim(explode('|',$l)[1]??''),'target'=>trim(explode('|',$l)[2]??'')],array_filter(array_map('trim',explode("\n",$fwc)))); } echo '<div class="grid gap-1.5">'; foreach($lnks as $lk){ $lbl=trim($lk['label']??''); $href=trim($lk['url']??''); if(!$lbl||!$href) continue; [$fhref,$auto]=Helper::menuUrl($href); $tgt=(($lk['target']??'')==='_blank'||$auto==='_blank')?' target="_blank" rel="noopener noreferrer"':''; echo '<a href="'.Helper::e($fhref).'"'.$tgt.' class="text-sm text-white/85 hover:text-white hover:translate-x-1 transition"><i class="fa fa-chevron-right mr-2 text-white"></i>'.Helper::e($lbl).'</a>'; } echo '</div>'; return; }
  if($t==='contact'){ echo '<div class="grid gap-1.5 text-sm"><p><i class="fa fa-location-dot w-5" style="color:var(--school-accent,#f59e0b)"></i>'.Helper::e(Database::setting('address','-')).'</p><p><i class="fa fa-phone w-5" style="color:var(--school-accent,#f59e0b)"></i>'.Helper::e(Database::setting('phone','-')).'</p><p><i class="fa fa-envelope w-5" style="color:var(--school-accent,#f59e0b)"></i>'.Helper::e(Database::setting('email','-')).'</p></div>'; return; }
  if($t==='social'){ echo '<div class="flex flex-wrap gap-2">'; foreach(['facebook'=>'fa-facebook-f','instagram'=>'fa-instagram','youtube'=>'fa-youtube','tiktok'=>'fa-tiktok'] as $fsn=>$fsi){$fsu=Database::setting($fsn,'');if(!$fsu)continue;echo '<a href="'.Helper::e($fsu).'" target="_blank" rel="noopener noreferrer" class="site-soc w-9 h-9 rounded-xl bg-white/10 grid place-items-center" aria-label="'.ucfirst($fsn).'"><i class="fab '.$fsi.'"></i></a>';} echo '</div>'; return; }
...
?>
<footer class="site-footer relative overflow-hidden mt-auto w-full" style="background:color-mix(in srgb,var(--school-primary,#059669) 88%,#020617);color:#e2e8f0">
<div class="absolute inset-x-0 top-0 h-1" style="background:linear-gradient(to right,var(--school-primary,#059669),var(--school-accent,#f59e0b),var(--school-primary,#059669))"></div>
<div class="absolute -top-32 -right-24 w-80 h-80 rounded-full blur-3xl pointer-events-none" style="background:color-mix(in srgb,var(--school-accent,#f59e0b) 18%,transparent)"></div>
...
<button id="toTop" aria-label="Kembali ke atas" class="site-totop fixed bottom-5 right-5 z-50 w-11 h-11 rounded-full text-white shadow-lg grid place-items-center opacity-0 invisible translate-y-3 transition-all duration-300" style="background:var(--school-primary,#059669)"><i class="fa fa-arrow-up"></i></button>
<style>.site-footer,.site-footer p,.site-footer a,.site-footer span,.site-footer div{color:#fff!important}.site-footer .text-white{color:#fff!important}.site-footer i.fa-chevron-right,.site-footer i.fa-tag{color:#fff!important;opacity:.9}.site-soc{color:#fff!important}.site-soc:hover{background:var(--school-primary,#059669)!important;color:#fff!important}.site-totop:hover{filter:brightness(1.12)}</style>
