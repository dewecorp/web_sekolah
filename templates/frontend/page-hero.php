...
<?php if($heroDesc): ?><p class="text-white/80 text-sm md:text-base max-w-2xl mt-4 text-justify leading-relaxed"><?= $heroDesc ?></p><?php endif; ?>
<?php if($heroStats||$heroActions): ?><div class="mt-5 flex flex-wrap gap-2 text-sm"><?= $heroActions ?><?php foreach($heroStats as $st): ?><span class="inline-flex items-center gap-2 <?= ($st['solid']??false)?'bg-white text-slate-900':'border border-white/40' ?> px-4 py-2 rounded-xl font-bold"><i class="fa <?= Helper::e($st['icon']??'fa-circle') ?> <?= ($st['solid']??false)?'text-emerald-600':'' ?>"></i><?= Helper::e($st['label']??'') ?></span><?php endforeach; ?></div><?php endif; ?>
</div>
</div>
